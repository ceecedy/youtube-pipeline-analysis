WITH comment_data AS (
    SELECT 
        *
    FROM {{ ref('staging_video_comments') }}  -- Reference to the staging model for raw YouTube comment data
),
...
comment_filled AS (
    ...
    FROM
        comment_data c
    LEFT JOIN 
        {{ ref('dim_videos') }} v  -- Reference to the dim_video table to get the corresponding video
    ON
...
SELECT 
    {{ dbt_utils.generate_surrogate_key([
        ...
        'cf.video_id', 
        'cf.published_at'
    ]) }} AS fact_comment_id,  -- Surrogate key based on the combination of comment_id, video_id, and published_at
    cf.comment_id,
    cf.video_id, 
    cf.comment_author,
    cf.comment_text,
